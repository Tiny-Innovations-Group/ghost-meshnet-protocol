...
9. [ ] **Sync `docs/overview.md` to 112 B canon.** Replace all 124 B / 248 B references with 112 B / 224 B. Remove the literal placeholder `(Include your gmp_core_layer.ksy and gmp_data_frame.ksy snippets here)`. Repoint to the real `.ksy` files in `/specs/`.
10. [ ] **Sync `docs/encryption-ratchet.md` to 112 B canon.** Replace the 124 B / 248 B quotes throughout. Replace the outdated inline `gmp_433_beacon.ksy` snippet (no enums, no bit-packing) with a link to `/specs/gmp_433_beacon.ksy`.
